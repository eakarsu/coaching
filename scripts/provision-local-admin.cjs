...
async function main() {
  ...
  try {
    const existing = await db.query('SELECT tenant_id,subject FROM coaching_identities WHERE lower(email)=lower($1)', [email]);
    ...
    await db.query('INSERT INTO coaching_tenants(id,name) VALUES($1,$2)', [tenantId, process.env.PROVISION_COMPANY_NAME || name]);
    await db.query("INSERT INTO coaching_identities(tenant_id,subject,role,email) VALUES($1,$2,'operator',$3)", [tenantId, subject, email]);
    await db.query('INSERT INTO coaching_local_credentials(tenant_id,subject,password_hash) VALUES($1,$2,$3)', [tenantId, subject, hashPassword(password)]);
