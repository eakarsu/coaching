...
async function login(email, password) {
  const result = await database().query(
    `SELECT i.tenant_id,i.subject,i.role,i.email,c.password_hash
       FROM coaching_identities i JOIN coaching_local_credentials c USING(tenant_id,subject)
  ...
  await database().query(
    `INSERT INTO coaching_local_sessions(token_hash,tenant_id,subject,expires_at) VALUES($1,$2,$3,now()+interval '8 hours')`,
...
async function session(token) {
  ...
  const result = await database().query(
    `UPDATE coaching_local_sessions s SET last_seen_at=now()
       FROM coaching_identities i
...
function requestToken(request) {
  ...
  const match = (request.headers.get('cookie') || '').match(/(?:^|;\s*)coaching_local_session=([^;]+)/);
